...
export class PostgresStore {
  ...
  async updateSubscription(id, input) {
    const result = await this.query(`update subscriptions set name=$1, description=$2, traffic_limit_bytes=$3, expires_at=$4, device_limit=$5, ip_limit=$6, reset_period=$7, support_url=$8, updated_at=now() where id=$9 returning *`, [input.name, input.description, input.trafficLimitBytes, input.expiresAt, input.deviceLimit, input.ipLimit, input.resetPeriod, input.supportUrl, id]);
    return result.rows[0] || null;
  }
...
export class MemoryStore {
  ...
  async updateSubscription(id, input) { const s = await this.getSubscriptionById(id); Object.assign(s, input); return s; }
